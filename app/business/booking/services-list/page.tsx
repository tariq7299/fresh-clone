...
export default function BookingPage() {
    ...
    const [currentSelectedTabIndex, setCurrentTabIndex] = useState<null | number>(null)
    const [currentActiveId, setCurrentActiveId] = useState<null | string>(null)
    console.log("tabsRefs", tabsRefs)

    useEffect(() => {


        if (currentActiveId && tabsRefs.current) {

            console.log("currentSelectedTabIndex", currentSelectedTabIndex)

            // tabsRefs.current?.[currentSelectedTabIndex].current.scrollIntoView({ behavior: 'smooth', block: 'center' })
            const element = tabsRefs.current.find(ref => {
                console.log("ref", ref)
                return ref.current?.innerText === currentActiveId
            })?.current
    ...
    const onChangeActiveId = (current, prev) => {
        ...
        // const element = tabsRefs.current.find(ref => {
        //     console.log("ref", ref)
        //     return ref.current?.innerText === current
        // })?.current
        // console.log("element", element)
        // element?.scrollIntoView({ behavior: 'smooth', block: 'center', inline: 'start' })

        setCurrentActiveId(current)
        ...
        <div className=" grid grid-cols-1 md:grid-cols-3  gap-6">
            ...
            <div className="col-span-3 lg:col-span-2" >
                ...
                <nav className="w-full bg-background sticky top-16 left-0">
                    <ScrollSpy onClickEach={onClickEach} onChangeActiveId={onChangeActiveId} activeClass="nav-active" activeAttr={true} offsetTop={0} offsetLeft={0} >
